...
inline const char * solverTypeLabel(solverType_e t) {
  switch (t) {
    ...
    case SOLVER_BT2:     return "BurrTools 2";
  }
...
inline const char * solverTypeCliNames(void) {
  return "\"BurrTools Classic\", crowell, \"Andrew Crowell\", bt2, \"BurrTools 2\"";
}
...
inline const char * solverTypeTooltip(void) {
  return " Solver engine. BurrTools Classic is the original complete take-apart "
         ...
         "disassemblies). BurrTools 2 is BurrTools Classic take-apart plus a "
         "dancing-cells assembly search split across threads (helps when "
         "assembly, not take-apart, is the limiter). ";
}
